    ...
    $.ajax({
        ...
        success: function (res) {
            ...
            for (var i = 0; i < res.length; i++) {
                var table = ['' + (i + 1) + '', '' + res[i].holidayName + '', '' + res[i].startDate + '', '' + res[i].endDate + ''];
                var data = { 'sl': (i + 1), 'oid': res[i].oid, 'holidayName': res[i].holidayName, 'startDate': res[i].startDate, 'endDate': res[i].endDate };
                    ...
                    { title: 'end date' },
                    {
                        ...
                        render: function (data, type, full, meta) {
                            return (
                                ...
                                'Edit</a>' +
                                '<a href="javascript:;" class="dropdown-item delete-record deleteDesignation" id="btnDelete" class="dropdown-item delete-record" id="btnDelete"  asp-action="Delete" asp-controller="Designation" asp-route-OID="@Model.OID">' +
...
function getNextDate(e) {
    ...
    $('#endDate').val(new Date(dates).toISOString().slice(0, 10));
}
...
$('body').on('click', '#btnEdit', function () {
    ...
    $('#oid').val(dta.oid);
    $('#holidayName').val(dta.holidayName);
    $('#startDate').val(dta.startDate);
    $('#endDate').val(dta.endDate);
    //$('#hStartdate').val(new Date(dta.startdate).toISOString().slice(0, 10));
    //$('#hEnddate').val(new Date(dta.enddate).toISOString().slice(0, 10));
