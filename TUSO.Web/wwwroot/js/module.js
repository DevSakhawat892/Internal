...
function ReadModule() {
   ...
   $.ajax({
      ...
      type: "GET",
      dataType: "json",
      cuntentType: "application/json",
      success: function (res) {
         console.log(res);
         for (var i = 0; i < res.length; i++) {
            var table = ['' + (i + 1) + '', '' + res[i].moduleName + '', '' + res[i].description];
            var data = { 'sl': (i + 1), 'oid': res[i].oid, 'moduleName': res[i].moduleName, 'description': res[i].description };
