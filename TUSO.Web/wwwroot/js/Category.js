    ...
    $.ajax({
        ...
        success: function (res) {
            ...
            for (var i = 0; i < res.length; i++) {
                ...
                var table = ['' + (i + 1) + '', '' + res[i].incidentTypeName + '', '' + res[i].description + '', '' + res[i].parentID];
                var data = { 'sl': (i + 1), 'oid': res[i].oid, 'incidentTypeName': res[i].incidentTypeName, 'description': res[i].description, 'parentID': res[i].parentID };
                    ...
                    { title: 'parent id' },
                    {
                        ...
                        render: function (data, type, full, meta) {
                            return (
                                ...
                                'Details</a>' +
                                '<a href="javascript:;" class="dropdown-item delete-record" id="btnDelete"  asp-action="Delete" asp-controller="Department" asp-route-OID="@Model.OID">' +
                                feather.icons['trash-2'].toSvg({ class: 'me-50 font-small-4' }) +
                                'Delete</a>' +
                                '</div>' +
                                '</div>' +
...
$('body').on('click', '#btnEdit', function () {
    ...
    $('#OID').val(dta.oid);
    $('#incidentTypeName').val(dta.incidentTypeName);
    $('#description').val(dta.description);
    $('#parentID').val(dta.parentID);
